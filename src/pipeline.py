from src.common import Logger, GameData, resolve_path
...
def run(input_text: str) -> str:
    ...
    """
    Logger.info("=" * 40)
    Logger.info("AlphaGameExplainer 开始运行")
    Logger.info("=" * 40)

    stockfish_path = resolve_path(
        os.getenv("STOCKFISH_PATH", "stockfish-windows-x86-64-avx2.exe")
    )
    ...
    moves = get_solution(board, stockfish_path)
    if not moves:
        Logger.warn("未能找到解法")
        return ""

    Logger.info("[3/5] SF标注 & 节点压缩...")
    compressed = compress(board, moves, stockfish_path)
