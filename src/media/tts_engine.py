...
def _clean_text_for_speech(text: str) -> str:
    ...
    # 全角分号/顿号类标点归一为逗号（08.04 修）。ChatTTS 词表不含「；」，
    # 日志会报 `found invalid characters: {'；'}` 并在该处行为未定义（跳读或
    # 即兴填充音）。决策管线的对比段天然爱用分号做并列（「甲…；乙…」），
    # 命中率比两条老管线高得多，故必须在此归一。语义等价（都是并列停顿），
    # 不改 seg.text——字幕保留分号，屏幕上更清晰。
    t = t.replace("；", "，").replace(";", "，")
    t = re.sub(r"[，,]{2,}", "，", t)
    t = re.sub(r"。{2,}", "。", t)
    t = re.sub(r"[，、：]+。", "。", t)
