...
PIECE_CN_DECOYS = {
    "后": ("之后", "随后", "然后", "过后", "而后", "其后", "此后", "事后",
           ...
           "后期", "后半", "后手", "王后方"),
    # 注：phase5 暴露的"侧后方/左后/右后/后方斜"等不再单列——decoy 用 str.replace
    # 顺序敏感，"后方"会先被替换，这些长词永远命中不到（死代码）。这类方位词的
    # "后"字最终会被"后方"等基词消除，残余再交 mentions_piece 的语境判断兜底。
...
def mentions_piece(text: str, cn_name: str) -> bool:
    ...
    PLAN-004 阶段C C1：扩展"后"字假阳性覆盖。phase5 全量回归暴露大量
    "X后"时间/方位助词被误判为棋子"后"（吃兵后/移动后/调整后/后方/侧后方/
    身后等）。分两步：(1) 抹掉扩展的助词复合词；(2) 对残留的"后"做语境判断——
    仅当前置字符是棋子语义字（白/黑/升变为/变成/多/有/出/造/得/新等）才判为
    棋子后，其余（方位/时间）放行。数据见 phase5 raw：假阳性 ctx 为"移动后"
    "后退""后面"，真幻觉 ctx 为"变成后""升变为后""多一个后"。
    """
    ...
    # PLAN-004 C1：仅"后"字需要语境判断（单字棋子里只有"后"有高频语法助词用法）。
    # 其他棋子字（车/象/马）复合词已被 decoy 表覆盖，残留即真提及。
    if cn_name != "后":
        return cn_name in stripped

    # PLAN-004 C1："后"字残留时的语境判断。
    # 真幻觉的"后"前面是棋子语义字符；假阳性的"后"前面是动词/方位/时间词。
    # 逐字符扫描残留文本中的每个"后"，看它前一个字符是否落在棋子语义集。
    #
    # 重要：本函数是 validate_material_existence 唯一依赖的程序化后验闸门
    # （ADR-015 双保险的"后验核查"那一半）。mentions_piece 漏判 = 整条程序化
    # 后验对该文本失效，只能靠 prompt 软约束兜底。因此对真幻觉取保守补集——
    # 宁可多拦少量模糊，也不漏放"造后"类幻觉。前置字符集已据 phase5 raw
    # 反向核查覆盖真实真幻觉表达。
    #
    # 棋子语义前置（出现这些就基本是讲棋子后），数据驱动来源见 phase5 raw：
    #   颜色：白/黑（白后/黑后）
    #   升变：变/成/为/升/出（变成后/升变为后/升变出后）
    #   数量/存在：多/有/无/缺/失/得/双/一/几/枚/个/只/那/这（多一个后/得一后）
    #   吃子/兑换：吃/兑/换/取/获/擒/斩/杀/夺（吃后/兑后/夺后）
    #   造子/动作：新/造/跳/冲/弃（新后/造后/跳后/冲后/弃后）
    queen_context_chars = set(
        "白黑变成为升降出"
        "多有无缺失得双一几枚个只那这"
        "吃兑换取获擒斩杀夺"
        "新造跳冲弃"
    ...
    for i, ch in enumerate(stripped):
        if ch != "后":
            continue
        # 句首"后"（i==0）：孤立"后"做主语几无歧义（"后跃至底线""后控制中心"），
        # 判为真提及，避免相对原 return cn_name in stripped 的起始位置回归。
        if i == 0:
            return True
        prev = stripped[i - 1]
        if prev in queen_context_chars:
            return True
    return False
