...
_EXAMPLE_BY_ENDGAME = {
    ...
    "单后杀王": (
        ...
        "总结：单后杀王的关键是保持后的安全距离避免逼和，用己方王配合后逐步把对方王逼至边线角落将杀。"
    ),
}
...
def _repair_failed_segments(backend, segments: list, chunk_nodes: list) -> Optional[dict]:
    ...
    for i, seg in enumerate(segments):
        node = chunk_nodes[i]
        ok, err = validate_single_segment(seg, node)
        if ok:
            continue
        ...
        if fixed_vo != original_vo:
            ...
            if validate_single_segment(seg, node)[0]:
                repaired_any = True
                continue

        prompt = _build_segment_repair_prompt(node, err)
        raw = backend.generate(prompt, grammar=SEGMENT_GRAMMAR)
        if not raw:
            continue
        repaired_seg = parse_single_segment(raw)
        if repaired_seg is None:
            continue
        repaired_vo = repaired_seg.get("voiceover", "")
        repaired_seg["voiceover"] = _auto_fix_voiceover(repaired_vo, node)
        repaired_ok, _ = validate_single_segment(repaired_seg, node)
        if repaired_ok:
...
def _compose_opening(storyboard: dict) -> str:
    ...
    try:
        ...
        text = clean_cjk_text("".join(parts))
...
def _fallback_summary(storyboard: dict) -> str:
    ...
    return clean_summary_text("".join(parts)) or f"总结一下，{endgame_name}重在稳扎稳打，逐步压缩对方空间。"
...
def generate_summary(storyboard: dict, backend, segments: list = None) -> str:
    ...
    # _fallback_summary 末尾也会经 clean_summary_text 剥掉前缀）统一补回，
...
def _endgame_post_process(commentary: GeneratedCommentary, all_segments: list,
                          nodes: list, storyboard: dict, backend) -> None:
    """残局后处理：将杀结论追加 + 跨段去重 + 总结/开场白生成。

    从 generate_structured 提取。直接操作传入的 commentary 与 all_segments。
    """
...
def generate_structured(board, storyboard: dict) -> GeneratedCommentary:
    ...
    from src.commentator.validators import validate_storyboard_chunk
    from src.infra.llm_backend import create_backend_from_env

    backend = create_backend_from_env()
    config = CommentaryConfig(
        build_header=_build_json_header,
        build_chunk_prompt=_endgame_chunk_prompt_wrapper(storyboard),
        build_grammar=build_chunk_grammar,
        validate_chunk=validate_storyboard_chunk,
        auto_fix_voiceover=_auto_fix_voiceover,
        repair_failed_segments=_repair_failed_segments,
        build_fallback_voiceover=_endgame_fallback_wrapper,
        post_process=_endgame_post_process,
    )
