...
- **docs/README.md**：docs 导航索引（交付前整理，2026-08-15）。
- **docs/工作小结-AlphaGameExplainer.docx**：项目交付工作小结（三阶段综述 + 突出点，对外交付用，Office 格式）。
- **docs/REVIEW-CHECKLIST-decision成片四维评审.md**：decision 成片四维评审清单（活跃）。
- **docs/adr/ADR-XXX.md**：架构决策完整论证（编号 006-022 连续；靠首行状态字段区分活跃 012/015-022 与已废弃 006-011/013/014，废弃决策不移走，符合 ADR 惯例）。
- **docs/plans/** 除 PLAN 外另含决策管线过程文档（交付前从 archive 并入，均已完成使命、保留可溯源）：HANDOFF-001/002（决策管线交接台账）、FINDINGS-002（方案评审与遗留问题）、多战略意图讲解方案设计与讨论记录。
